...
int mtk_cfg80211_get_station(struct wiphy *wiphy, struct net_device *ndev, u8 *mac, struct station_info *sinfo)
{
	 ...
	 * wificond will bring the MAC address of device itself to
	 * retrieve TX/RX statistics */
	if (UNEQUAL_MAC_ADDR(arBssid, mac) &&
		UNEQUAL_MAC_ADDR(ndev->dev_addr, mac)) {
		/* wrong MAC address */
		DBGLOG(REQ, WARN,
			"Incorrect MAC addr[" MACSTR "], device[" MACSTR "],"
			"currently connected BSSID[" MACSTR "]\n",
			MAC2STR(mac), MAC2STR(ndev->dev_addr),
			MAC2STR(arBssid));
		return -ENOENT;
	}
