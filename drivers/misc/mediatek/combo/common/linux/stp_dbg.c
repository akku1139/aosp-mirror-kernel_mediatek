...
static INT32 wcn_gzip_compressor(void *worker, UINT8 *in_buf, INT32 in_sz, UINT8 *out_buf,
				 INT32 *out_sz, INT32 finish)
{
	INT32 ret = 0;
		...
		if (0 != gStpDbgLogOut) {
			...
			pHdr =
			    (STP_DBG_HDR_T *) &(stp_dbg->logsys->queue[stp_dbg->logsys->in].
						 buffer[0]);
			pBuf =
			    (char *)&(stp_dbg->logsys->queue[stp_dbg->logsys->in].buffer[0]) +
...
int stp_dbg_dmp_print(MTKSTP_DBG_T *stp_dbg)
{
...
static int
stp_dbg_add_pkt(MTKSTP_DBG_T *stp_dbg, struct stp_dbg_pkt_hdr *hdr, const unsigned char *body)
{
...
int stp_dbg_log_pkt(MTKSTP_DBG_T *stp_dbg, int dbg_type,
		    int type, int ack_no, int seq_no, int crc, int dir, int len,
		    const unsigned char *body)
{
	...
	if (stp_dbg->is_enable == 0) {
		...
		stp_dbg_fill_hdr(&hdr,
				 (int)type,
				 (int)ack_no,
				 (int)seq_no, (int)crc, (int)dir, (int)len, (int)dbg_type);
...
char stp_dbg_nl_send(char *aucMsg, unsigned char cmd, INT32 len)
{
...
INT32 stp_dbg_set_version_info(UINT32 chipid, UINT8 *pRomVer, UINT8 *wifiVer, UINT8 *pPatchVer,
			       UINT8 *pPatchBrh)
{
	if (g_stp_dbg_cpupcr) {
