...
int ftxxxx_i2c_Write(struct i2c_client *client, char *writebuf, int writelen)
{
	...
	if (retry == IICReadWriteRetryTime) {
		...
		if (IICErrorCountor >= 10) {
			pr_info("%s: i2c read/write error over 10 times\n",
								__func__);
			pr_info("%s: excute reset IC process\n",
								__func__);
			return ret;
		}
		return ret;
...
static int fts_report_value(struct ts_event *data)
{
	...
	if (ret < 0) {
		pr_info("[Focal] %s read touchdata failed.\n", __func__);
	...
	for (i = 0; i<(buf.fingers) && (i<CFG_MAX_TOUCH_POINTS); i++, touch++) {
		...
		if (touch->event == 0 || touch->event == 2) {
			...
			/*
			pr_info("[Focal] %d: id=%d, x=%d, y=%d, p=0x%x, m=%d\n",
...
static irqreturn_t tpd_eint_interrupt_handler(int irq, void *dev_id)
{
	//pr_info("TOUCH interrupt has been triggered\n");
	TPD_DEBUG_PRINT_INT;
	tpd_flag = 1;
...
static int fts_init_gpio_hw(void)
{
	...
	if (ret) {
		pr_info("[Focal] reset-gpio:%d request fail.\n",
						tpd_rst_gpio_number);
		return ret;
	}
	ret = gpio_request(tpd_int_gpio_number, "ft5726-int");
	if (ret) {
		pr_info("[Focal] int-gpio:%d request fail.\n",
						tpd_int_gpio_number);
		return ret;
	}
	gpio_direction_output(tpd_rst_gpio_number, 0);
	pr_info("[Focal] rst-gpio = %d, int-gpio = %d\n",
...
static void tpd_power_on(void)
{
	...
	if (ret != 0)
		pr_info("[Focal] Failed to enable reg-vgp6: %d\n", ret);
	ret = regulator_enable(tpd->io_reg);
	if (ret != 0)
		pr_info("[Focal] Failed to enable reg-vgp4: %d\n", ret);
...
static int of_get_focal_platform_data(struct device *dev)
{
	if (dev->of_node) {
		...
		if (!match) {
			pr_info("%s: No device match found\n", __func__);
			return -ENODEV;
		}
	}
	...
	pr_info("[Focal] tpd_rst_gpio= %d, tpd_int_gpio= %d, irq = %d\n",
					...
					const struct i2c_device_id *id)
{
	...
	ftxxxx_i2c_Read(i2c_client, &reg_addr, 1, &reg_value[0], 1);

	reg_addr = FTS_REG_FW_VER;
	ftxxxx_i2c_Read(i2c_client, &reg_addr, 1, &reg_value[1], 1);

	reg_addr = FTS_REG_CHIP_ID;
	retval = ftxxxx_i2c_Read(i2c_client, &reg_addr, 1, &reg_value[2], 1);
	pr_info("[Focal] Report Rate:%dHz , FW ver:0x%x, Chip id:0x%x\n",
			reg_value[0] * 10, reg_value[1], reg_value[2]);
	if ( retval < 0 ) {
		pr_info("[Focal] Read I2C error! driver NOt load!!\n");
		return 0;
	}
	...
	if (retval < 0)
		pr_info("[Focal] request_irq faled.");

	thread = kthread_run(touch_event_handler, 0, TPD_DEVICE);
	if (IS_ERR(thread)) {
		retval = PTR_ERR(thread);
		pr_info("[Focal] failed to create kernel thread: %d\n", retval);
	}
	...
	if (fts_rw_iic_drv_init(i2c_client) < 0)
		pr_info("%s: create fts control iic driver failed\n", __func__);
#endif
	...
	pr_info("*****Enter Focal Touch Auto Upgrade*****\n");
...
{
	int ret;
	pr_info("force_reset_guitar\n");
	ret = regulator_disable(tpd->reg);
	if (ret != 0)
		pr_info("Failed to disable reg-vgp6: %d\n", ret);
	ret = regulator_disable(tpd->io_reg);
	if (ret != 0)
		pr_info("Failed to disable reg-vgp4: %d\n", ret);
	msleep(200);
	ret = regulator_enable(tpd->reg);
	if (ret != 0)
		pr_info("Failed to enable reg-vgp6: %d\n", ret);
	ret = regulator_enable(tpd->io_reg);
	if (ret != 0)
		pr_info("Failed to enable reg-vgp4: %d\n", ret);
	msleep(20);
	gpio_direction_output(tpd_rst_gpio_number, 0);
	msleep(20);
	pr_info(" fts ic reset\n");
	gpio_set_value(tpd_rst_gpio_number, 1);
	msleep(300);
}
...
static void gtp_esd_check_func(struct work_struct *work)
{
	...
	if (i >= 3) {
		force_reset_guitar();
		pr_info("focal ret = %d, A3_Reg_Value = 0x%02x\n", ret, data);
		reset_flag = 1;
	...
	if (1 == flag_error) {
		pr_info("focal--tpd reset. data=%d count_irq\n",
						data, count_irq);
		force_reset_guitar();
		reset_flag = 1;
	...
	if (0x01 == g_first_read_91) {
		...
		if (g_old_91_Reg_Value == data) {
			...
			pr_info("focal g_91value_same_count=%d\n",
						g_91value_same_count);

			if (RESET_91_REGVALUE_SAMECOUNT==g_91value_same_count) {
				force_reset_guitar();
				pr_info("focal g_91value_same_count = 5\n");
...
{
	pr_info("[Focal] fts I2C Driver init.\n");

	if (i2c_add_driver(&tpd_i2c_driver)!=0) {
		pr_info("[Foacl] fts unable to add i2c driver.\n");
		return -1;
	}
	...
	pr_info("[Focal] fts add i2c driver ok.\n");
	return 0;
}
...
{
	pr_info("[Focal] %s-%d TOUCH RESUME.\n", __func__, __LINE__);
...
#endif
	focal_suspend_flag = false;
	pr_info("[Focal] %s-%d TOUCH RESUME DONE.\n", __func__, __LINE__);
}
...
static void tpd_suspend(struct device *h)
{
	...
	if (focal_suspend_flag) {
		pr_info("[Focal] IC already suspend\n");
		return;
	}
	pr_info("[Focal] %s-%d TOUCH SUSPEND.\n", __func__, __LINE__);
	...
	pr_info("[Focal] %s-%d TOUCH SUSPEND DONE.\n", __func__, __LINE__);
}
...
{
	pr_info("[Focal] MediaTek fts touch panel driver init\n");

	if (tpd_driver_add(&tpd_device_driver) < 0)
		pr_info("[Focal] MediaTek add fts driver failed\n");
...
{
	pr_info("[Focal] MediaTek fts touch panel driver exit\n");
